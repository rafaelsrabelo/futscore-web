...
export async function getAdminAthleteMatches(
  ...
  const path = `/admin/athletes/${athleteId}/matches${query ? `?${query}` : ""}`;
  const url = `${API_URL}${path}`;
  let res: Response;
  try {
    res = await fetchAuthed(path, { cache: "no-store" });
  } catch (err) {
    console.error("[athlete-matches] network error", { url, err });
    return { kind: "network-error", url };
  }
  if (res.status === 401 || res.status === 403) {
    return { kind: "auth-error", status: res.status as 401 | 403 };
  ...
  if (!res.ok) {
    const body = await res.text().catch(() => "");
    console.error("[athlete-matches] not ok", {
      status: res.status,
      url,
      body: body.slice(0, 300),
    });
  ...
  return { kind: "ok", data: (await res.json()) as AdminMatchListResponse };
}
